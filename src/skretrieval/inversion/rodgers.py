...
class Rodgers(Minimizer):
    ...
    def retrieve(
        ...
        for iter_idx in range(self._max_iter):
            ...
            logging.info("chi_sq: %s (%s)", str(chi_sq), str(chi_sq_only_meas))
            logging.info(
                "expected_chi_sq: %s (%s)",
                str(chi_sq_linear),
                str(chi_sq_only_meas_linear),
            )
            ...
            logging.info("dcost: %s", str(dcost))
            ...
            if chi_sq_prev is not None and chi_sq_prev < chi_sq:
                ...
                logging.info(
                    "Iteration was worse increasing LM factor to %s", self._lm_damping
                )
                ...
                if self._iterative_update_lm and iter_idx > 0:
                    ...
                    logging.info(
                        "Iteration was better decreasing LM factor to %s",
                        self._lm_damping,
                    )
            ...
            if (
                self._convergence_check_method.lower() == "linear"
                and chi_sq / chi_sq_linear < self._convergence_factor
                and chi_sq / chi_sq_linear > 1
            ):
                logging.info(
                    "Stopping due to early convergence. Convergence Ratio: %s",
                    chi_sq / chi_sq_linear,
                )
                break
            if (
                self._convergence_check_method.lower() == "dcost"
                and dcost < self._convergence_factor
            ):
                logging.info(
