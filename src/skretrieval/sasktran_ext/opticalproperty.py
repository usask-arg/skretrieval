...
class OpticalPropertyLineShape(sk.OpticalProperty):
    ...
    """
    def __init__(self, optical_property: sk.OpticalProperty, line_shape_fn: Callable[[float], LineShape],
        ...
        if self._wavel_to_calc_for is not None:
            self._iskopticalproperty = self.convolved_optical_property(self._unconv_optical_prop,
                                                                       self._line_shape_fn,
                                                                       self._wavel_to_calc_for)

    def skif_object(self, **kwargs):
        if len(self._valid_wavelengths) != len(kwargs['engine'].wavelengths) or not np.allclose(self._valid_wavelengths, kwargs['engine'].wavelengths):
            self._valid_wavelengths = kwargs['engine'].wavelengths

            calc_wavelengths = copy(self._valid_wavelengths)
            calc_wavelengths = np.concatenate(([np.nanmin(calc_wavelengths) - 1], calc_wavelengths,
    ...
    def convolved_optical_property(hires_optprop: sk.OpticalProperty, line_shape_fn: Callable[[float], LineShape],
                                   wavel_to_calc_for):
        """
        ...
        spec_resolution = hires_optprop.info['spectral resolution'] = 0.02

        for T, ra in zip(temp, temp_range):
            # Get the high resolution DBM cross section
            clim_temp = sk.ClimatologyUserDefined([0, 10000], {'SKCLIMATOLOGY_TEMPERATURE_K': [T, T]}, interp='linear')
            ...
            xsect = hires_optprop.calculate_cross_sections(clim_temp, 0, 0, 1000, 54372, wavel_hr)
            ...
            good = xsectconv > 0
            if np.nansum(good) > 0:
                full_xsect = np.interp(wavel_hr, wavelconv[good], xsectconv[good], left=np.nan, right=np.nan)

                optprop.AddUserDefined(T, wavel_hr[~np.isnan(full_xsect)], full_xsect[~np.isnan(full_xsect)])
