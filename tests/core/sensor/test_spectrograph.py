...
def test_spectrograph():
    ...
    model_geometry = sk2.Geometry1D(np.cos(np.deg2rad(60)), 0, 6372000, alt_grid, sk2.InterpolationMethod.LinearInterpolation,
    ...
    atmosphere = sk2.Atmosphere(model_geometry, config, wavelengths_nm=np.arange(280, 800, 0.1))
    ...
    radiance.data["look_vectors"] = (["los", "xyz"], np.vstack([los.look_vector for los in geo.lines_of_sight]))

    wavel_grid = np.arange(280, 800, 1.0)

    pixel_shape = [Gaussian(fwhm=1) for _ in wavel_grid]

    spectrograph = Spectrograph(wavel_grid, pixel_shape, Gaussian(fwhm=1), Rectangle(1, mode="constant"))

    optical_geo = OpticalGeometry(observer=geo.lines_of_sight[10].observer,
                                  look_vector=geo.lines_of_sight[10].look_vector,
                                  mjd=geo.lines_of_sight[10].mjd,
                                  local_up=geo.lines_of_sight[10].observer / np.linalg.norm(geo.lines_of_sight[10].observer)
                                  )

    modelled_radiance = spectrograph.model_radiance(radiance=radiance, orientation=optical_geo)
