...
class PointingAlgorithms:
    ...
    def set_limb_boresight_to_look_at_tangent_altitude(self,
        ...
        if (ok):
            ...
            look = self._geo.from_tangent_altitude(target_tangent_altitude, observer, horiz)
    ...
    def apply_roll_control(self,
        ...
        if (roll_cntl == 'limb'):                                                   # roll control is limb
            self._geo.from_xyz(target_location)                                     # so zero roll occurs when the Instrument Z axis, is as parallel as possible to the local up unit vector
            zunit = self._geo.local_up                                              # We want to choose a z unit vector. Get local up at the tangent point
            ...
            self._geo.from_xyz(target_location)                                     # so zero roll occurs when the Instrument Z axis, is as parallel as possible to the local north
            zunit = -self._geo.local_south                                          # We want to choose a z unit vector. Get local north at the target location
            ythreshold = math.sin(math.radians(0.5))
            xunit = lookvector                                                      # Get the look vector unit vector as the X direction
            yunit = np.cross(zunit, xunit)                                          # Get the y unit vector which points towards the left as seen from the oobserver.
            ymag = np.linalg.norm(yunit)                                            # Get the magnitude of the cross product. This becomes
            ...
            self._geo.from_xyz(observer_location)                                   # so zero roll occurs when the Instrument Z axis, is as parallel as possible to the local up unit vector
            ...
            if (horizontalunitvector is not None):                                  # If the caller has passed in the horizontal unit vector of the line of sight
                zunit = self._geo.local_up                                          # then we can use it with good stability, so get the z unit vector
                ...
                zunit = self._geo.local_up                                          # then we try our best. We want to choose a z unit vector. Get local up at the tangent point
                ...
                if (math.fabs(z) > 0.9999619):                                      # If we have almost no horizontal component( within 0.5 degrees of straight up/down)
                    horiz = -self._geo.local_south                                  # then arbitrarily use the north direction
...
class OrientationTechniques:
    ...
    def __init__(self):
        ...
        self._lookvector_convertors: Dict[str, Tuple[Callable[[PointingAlgorithms, np.ndarray, str], bool], List[int]]] = {}    # A dictionary of orientation techniques. Each entry of the form [name, function]
        ...
        self._look_definitions: List[Tuple[str, Union[np.ndarray, int], str]] = []                                              # A list of various representations of look vector specifications. Each entry of the form [technique, data_array, roll_control]
        self._turntable_data: List[np.ndarray] = []                                                                             # A list of instrument internal orientation arrays of azimuth, elevation and roll in the instrument control frame.
        ...
        self.add_lookvector_convertor('tangent_xyz_look', _technique_set_limb_look_vectors_from_unit_xyz, [3, 4])       # Set look vectors using ITRF/ecef xyz location vectors
        self.add_lookvector_convertor('tangent_altitude', _technique_set_look_vectors_from_tangent_altitude, [2, 3])    # specifies
        self.add_lookvector_convertor('tangent_from_orbitplane', _technique_set_boresight_look_at_location_orbitangle, [2, 3])
        self.add_lookvector_convertor('location_xyz', _technique_set_boresight_look_at_location_xyz, [3, 4])
        self.add_lookvector_convertor('location_llh', _technique_set_boresight_look_at_location_llh, [3, 4])
        self.add_lookvector_convertor('unit_vectors', _technique_set_boresight_pointing_from_unitvectors, [6])
        self.add_lookvector_convertor('azi_elev', _technique_set_observer_to_look_in_azi_elev, [2, 3])
        self.add_lookvector_convertor('yaw_pitch_roll', _technique_set_observer_to_look_in_azi_elev, [2, 3])
        self.add_lookvector_convertor('from_platform', _technique_set_platform_pointing_from_platform, [0])

    # ------------------------------------------------------------------------------
    #           add_lookvector_convertor
    # ------------------------------------------------------------------------------
    def add_lookvector_convertor(self, looktype: str, convertor_function: Callable[[PointingAlgorithms, np.ndarray, str], bool], num_user_params: List[int]):
    ...
    def _check_position_converter_key(self, key: str) -> bool:
        ...
        if not ok:
            raise ValueError('The position converter keyword <{:s}> is not in the position converter dictionary. You may need to manually add this function'.format(key))
        return ok
    ...
    def _check_look_vector_converter_key(self, key: str) -> bool:
        ...
        if not ok:
            raise ValueError('The lookvector converter keyword <{:s}> is not in the position converter dictionary. You may need to manually add this function'.format(key))
        return ok
    ...
    def _decode_orientation_entry(self, look_vectors):
        ...
        if (key == 'from_platform'):
            data = None
            roll_control = 'undefined'
        self._check_look_vector_converter_key(key)                                              # check that the look-vector technique is supported
        num_params = self._lookvector_convertors[key][1]                                        # get the number of parameters expected from this look vector technique
    ...
    def add_measurement_set(self,
                            ...
                            observer_positions: Tuple[str, Union[Sequence, np.ndarray]],
                            look_vectors: Tuple[str, str, Union[Sequence, np.ndarray]],
                            instrument_internal_rotation: Union[Sequence, np.ndarray, None] = None
                            ):
        """
        ...
        **N* distinct settings of *time*, *position* and *orientation*. For convenience, settings for one variable can be represented
        ...
        numpositions, positiondata, positionkey = self._decode_position_entry(observer_positions)       # Get the number of positions, the key
        numlooks, lookdata, orientkey, roll_control = self._decode_orientation_entry(look_vectors)
        numturntable, turntabledata = self._decode_instrument_internal_rotation(instrument_internal_rotation)
        ...
        if (numtimes == 1):
            ...
            if (numtimes != maxpts):
                raise ValueError('The number of time parameters given {} does not equal 1 or the number of looks {} and  and orientations given {}'.format(numtimes, numpositions, numlooks))
        ...
        if (positionkey == 'from_platform'):                                                            # if the technique parameter array returns as None
            ...
            if (numpositions != maxpts):
                raise ValueError('The number of position parameters given {} does not equal 1 or the number of times {} and orientations given {}'.format(numpositions, numtimes, numlooks))
        ...
        if (orientkey == 'from_platform'):                                                              # if the technique parameter array returns as None
            ...
            if (numlooks != maxpts):
                raise ValueError('The number of orientation parameters given {} does not equal 1 or the number of times {} and positions {} given'.format(numlooks, numtimes, numpositions))
        ...
        self._look_definitions.append((orientkey, lookdata, roll_control))                              # append these look vectors definitions to the internal measurement set within thgis object.
        ...
        for iseg in range(len(self._utc)):                                                                              # Loop through all of the time segements. There is usually just one
            ...
            for it in range(numutc):                                                                                    # Loop over all of the time values, we have already validated that the total number of positions and look arrays are the same size as the time array
                ...
                if (iturn >= numturn):                                                                                  # if we have exceeded the bounds of the current segment
                    ...
                    currentturn = self._turntable_data[iturnseg]                                                   # get the name of the position technique and the current position technique data for this segment
                    iturn = 0                                                                                           # Reset our position in the current segment
                    numturn = currentturn.shape[0]                                                                      # then get the number of positions ion this segment
                thisturn = currentturn[iturn, :]                                                                        # get the technique parameter data for this measurement point.
                ok3 = _technique_set_instrument_internal_orientation(platform, utc, thisturn)                           # Call the positioning technique function. Its sets the Platform position State. It returns false if it fails.
                ...
                if (ilook >= numlook):                                                                                  # if we have exceeded the bounds of the current segment
                    ...
                    looktype, currentlook, roll_control = self._look_definitions[ilookseg]                              # get the technique, parameter data and roll control for this settings
                    lookfunc = self._lookvector_convertors[looktype][0]                                                 # get the look technique function
                ...
                if (ok1 and ok2 and ok3):
                    ...
                    logging.warning('Rejecting point {} from the observation policy. Positioning technique={} status = {}, Pointing Technique={} status = {}, Internal Rotation status = {}'.format(iutc, postype, ok1, looktype, ok2, ok3))
                    allok = False
        if not allok:
