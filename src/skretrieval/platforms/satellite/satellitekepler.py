...
from math import cos, sin, radians, sqrt, pi, fabs, pow, atan2, nan, acos, degrees, log10, floor
import numpy as np
...
class SatelliteKepler(SatelliteBase):
    ...
    def __init__(self, ut: Union[np.datetime64, datetime, float, str],
    ...
    def sun_synchronous_inclination(self, semi_major_axis_meters: float) -> float:
        ...
        if (cosi < -1):
            raise Exception('Orbit semi major axis must be less than 12352 km for sun sync to work')
        return acos(cosi)
    ...
    def update_eci_position(self, autc: Union[np.datetime64, datetime]):
        """
...
#        print(line2)
    ...
    def from_elements(self,
        ...
        if period_from_seconds is not None:
            assert period_from_altitude is None, "You cannot set both period_from_altitude and period_from_seconds"
        ...
        if longitude_of_ascending_node_degrees is not None:
            ...
            st = gmst(utc)
            ra = st + longitude_of_ascending_node_degrees / 15.0                # Get RA at the longitude in hours (0-24)
            if ra >= 24.0:
    ...
    def from_state_vector(self, autc: datetime, orbitnumber: int, r: np.ndarray, v: np.ndarray):
        """
        ...
        print("Must update a few extra orbit parameters inside this code")
        utc = skretrieval.time.ut_to_datetime(autc)
        self.m_epoch = utc
        h = np.cross(r, v)                                                  # Get the angular momentum vector (which is constant for the orbit)
        runit = UnitVector(r)                                               # Get the radial unit vector
        self.m_h = Magnitude(h)                                             # Get the magnitude of the angular ecivelocity
        self.m_zunit = UnitVector(h)                                        # Get the angular momementum unit vector (aka z axis of the orbital plane)
        ev = np.cross(v, h) / self.m_mu - runit                             # Get the eccentricity vector
        self.m_e = Magnitude(ev)                                            # Get the eccentricity of the ellipse.
        if (self.m_e <= 0.0):                                               # if the eccentricity is zero (or even less due to round off
            self.m_e = 0.0                                                  # set the eccentricity to 0
            ev = runit                                                      # and let the perigree be at this point
        self.m_xunit = UnitVector(ev)                                       # Get the eccentricity unitvector which points to the perigree (ie 0 degrees Mean Anomaly)
        self.m_yunit = np.cross(self.m_zunit, self.m_xunit)                 # Get the y axis of the orbital plane from z cross x
        ...
        self.m_N0 = sqrt(self.m_mu / (self.m_a * self.m_a * self.m_a))      # Get the mean motion  (radians per second)

        x = np.dot(r, self.m_xunit)                                         # Get the x component parallel to the semi-major axis
        y = np.dot(r, self.m_yunit)                                         # Get the y component parallel to the semi-minor axis
        sinE = y / self.m_b
