...
class StateVectorElementConstituent(StateVectorElement, sk2.constituent.base.Constituent):
    def __init__(self,
        ...
        for property_name in self._property_names:
            self._prior_dict[property_name] = copy(getattr(self._constituent, property_name))
    ...
    def inverse_apriori_covariance(self) -> np.ndarray:
        ...
        for property_name in self._property_names:
            ...
            if self._log_space:
                prior_mats.append(np.linalg.inv(toeplitz(corel) * self._prior_influence[property_name]**2))
            else:
                prior_mats.append(np.linalg.inv(toeplitz(corel) * np.outer(self._prior_dict[property_name], self._prior_dict[property_name]) * self._prior_influence[property_name]**2))
            ...
            if property_name in self._first_order_tikh:
                gamma = two_dim_vertical_first_deriv(1, len(val)) * (1/self._first_order_tikh[property_name])
    ...
    def propagate_wf(self, radiance: xr.Dataset) -> xr.Dataset:
        if "extinction_per_m" in self._property_names:
            radiance = radiance.rename({f"wf_{self._constituent_name}_extinction": f"wf_{self._constituent_name}_extinction_per_m"} )
        wfs = []
        for property_name in self._property_names:
            wfs.append(radiance[f"wf_{self._constituent_name}_{property_name}"].rename({radiance[f"wf_{self._constituent_name}_{property_name}"].dims[0]: "x"}))
