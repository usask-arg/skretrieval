from skretrieval.retrieval import Minimizer, RetrievalTarget, ForwardModel
...
class Rodgers(Minimizer):
    def __init__(self, max_iter: int = 10, lm_damping: float = 0, iterative_update_lm: bool = False,
                 retreat_lm: bool = False, lm_change_factor: float = 1.5, convergence_factor: float = 1,
                 convergence_check_method='linear', lm_damping_method='fletcher'):
        """
    ...
    def _measurement_parameters(retrieval_target: RetrievalTarget, measurement_l1):
        ...
        if 'y_error' in y_meas_dict:
            ...
            if len(np.shape(y_meas_dict['y_error'])) == 1:
                ...
                Sy = sparse.csc_matrix(sparse.diags(y_meas_dict['y_error'][good_meas], 0))
                inv_Sy = sparse.csc_matrix(sparse.diags(1 / y_meas_dict['y_error'][good_meas], 0))
            else:
    ...
    def retrieve(self, measurement_l1, forward_model: ForwardModel, retrieval_target: RetrievalTarget):
        retrieval_target.initialize(forward_model, measurement_l1)

        output_dict = dict()

        y_meas_dict, y_meas, Sy, inv_Sy, good_meas = self._measurement_parameters(retrieval_target, measurement_l1)
        ...
        for iter_idx in range(self._max_iter):
            ...
            if sparse.issparse(K):
                if self._lm_damping_method.lower() == 'fletcher':
                    A = K.T @ inv_Sy @ K + self._lm_damping * sparse.diags((K.T @ inv_Sy @ K).diagonal()) + inv_Sa
                elif self._lm_damping_method.lower() == 'prior':
                    A = K.T @ inv_Sy @ K + (self._lm_damping + 1) * inv_Sa
                elif self._lm_damping_method == 'identity':
                    A = K.T @ inv_Sy @ K + inv_Sa + self._lm_damping * np.eye(inv_Sa.shape)
                else:
                    raise ValueError('lm_damping_method should be one of fletcher, prior, or identity')
            else:
                if self._lm_damping_method.lower() == 'fletcher':
                    A = K.T @ inv_Sy @ K + self._lm_damping * np.diag(np.diag(K.T @ inv_Sy @ K)) + inv_Sa
                elif self._lm_damping_method.lower() == 'prior':
                    A = K.T @ inv_Sy @ K + (self._lm_damping + 1) * inv_Sa
                elif self._lm_damping_method == 'identity':
                    A = K.T @ inv_Sy @ K + inv_Sa + self._lm_damping * np.eye(inv_Sa.shape)
            ...
            chi_sq_only_meas_linear = (y_meas - y_ret - K @ delta_x_without_lm).T @ inv_Sy @\
                                      (y_meas - y_ret - K @ delta_x_without_lm)

            chi_sq_linear = (y_meas - y_ret - K @ delta_x_without_lm).T @ inv_Sy @\
                            (y_meas - y_ret - K @ delta_x_without_lm) +\
                            (x_a - x - delta_x_without_lm).T @ inv_Sa @ (x_a - x - delta_x_without_lm)

            chi_sq_only_meas /= len(y_meas)
            chi_sq /= len(y_meas)

            if np.isnan(chi_sq) or np.isinf(chi_sq):
                raise ValueError('chi_sq is infinite or nan')

            chi_sq_only_meas_linear /= len(y_meas)
            chi_sq_linear /= len(y_meas)

            logging.info('chi_sq:' + str(chi_sq) + '(' + str(chi_sq_only_meas) + ')')
            logging.info('expected_chi_sq:' + str(chi_sq_linear) + '(' + str(chi_sq_only_meas_linear) + ')')

            retrieval_target.update_state(x_new)

            dcost = delta_x_without_lm @ (K.T @ inv_Sy @ (y_meas - y_ret) + inv_Sa @ (x - x_a)) / len(x)
            logging.info('dcost: {}'.format(dcost))
            ...
            if chi_sq_prev is not None and chi_sq_prev < chi_sq:
                ...
                logging.info("Iteration was worse increasing LM factor to {}".format(self._lm_damping))
                ...
                if self._iterative_update_lm and iter_idx > 0:
                    ...
                    logging.info("Iteration was better decreasing LM factor to {}".format(self._lm_damping))

            xs.append(retrieval_target.state_vector())


            if self._convergence_check_method.lower() == 'linear':
                if chi_sq / chi_sq_linear < self._convergence_factor and chi_sq / chi_sq_linear > 1:
                    logging.info("Stopping due to early convergence. Convergence Ratio: {}".format(chi_sq / chi_sq_linear))
                    break
            elif self._convergence_check_method.lower() == 'dcost':
                if dcost < self._convergence_factor:
                    logging.info("Stopping due to early convergence. dcost is less than convergence_factor")
                    break

            if iter_idx != self._max_iter - 1:
                predicted_delta_y = y_meas - y_ret - K @ delta_x_without_lm
                retrieval_target.adjust_parameters(forward_model, y_ret_dict, chi_sq, chi_sq_linear, iter_idx, predicted_delta_y)

                if retrieval_target.state_vector_allowed_to_change():
                    x_a, inv_Sa, initial_guess = self._apriori_parameters(retrieval_target)
                if retrieval_target.measurement_vector_allowed_to_change():
                    y_meas_dict, y_meas, Sy, inv_Sy, good_meas = self._measurement_parameters(retrieval_target,
                                                                                              measurement_l1)
