...
class TestSpectrographIntegration(unittest.TestCase):
    def setUp(self):
        self.optical_geometry = OpticalGeometry(observer=[0, 6392 * 1000, 6392 * 1000],
                                                look_vector=[0, -1, 0],
                                                local_up=[0, 0, 1],
                                                mjd=54372)
        ...
        self.spectrograph = Spectrograph(self.wavel_nm, line_shape, DeltaFunction(), DeltaFunction())

    def test_radiance(self):
        geo = sk.Geometry()
        geo.lines_of_sight = self.spectrograph.measurement_geometry(self.optical_geometry)
        ...
        convolve_rad = self.spectrograph.model_radiance(self.optical_geometry, engine.wavelengths, engine.geometry, rad)
    ...
    def test_weighting_function(self):
        ...
        convolve_wf = self.spectrograph.model_radiance(self.optical_geometry, engine.wavelengths, engine.geometry, rad, wf)
