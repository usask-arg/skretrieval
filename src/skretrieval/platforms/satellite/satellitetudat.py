...
class StateCache:
    ...
    def __init__(self):
        ...
        self.timeoffsets: np.ndarray = None                                         # The array of time offsets in seconds since J2000.
        self.positions: np.ndarray = None                                           # The array of platform_ecef_positions in the current simulation window
        self.velocities: np.ndarray = None                                          # The array of velocities in the current simulation window
        self.ts: skyfield.timelib.Timescale = skyfield.api.load.timescale()         # Load in the skyfield timescales
        self.j2000: skyfield.timelib.Time = self.ts.tt(2000, 1, 1, 12, 0, 0)        # Keep the time of J2000 cached
    ...
    def datetime_to_ttj2000(self, mjd: datetime) -> float:
        ...
        t = self.ts.observation_policy.utc(mjd.year, mjd.month, mjd.day, mjd.hour, mjd.minute, mjd.second + mjd.microsecond / 1.0E6)
        tt = (t.tt - self.j2000.tt) * 86400.0
        return tt
    ...
    def interpolate_current_entry(self, tt_j2000: float) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def interpolate_position_and_velocity(self, tt_j2000: float) -> Tuple[np.ndarray, np.ndarray]:
...
class SatelliteTudat(SatelliteBase):
        ...
        if (self._m_time is None) or (mjd != self._m_time):
            ...
            while (not ok):                                                                 # so
                if (self.cache.is_valid()):                                                 # If we have a valid cache
                    if (tt_j2000 >= self.cache.cache_endtime):                              # then if we are going forwards in time
                        startdt = self.cache.cache_endtime                                 # start at the end of the current cache
                    ...
                    raise Exception('You must initialize the tudat satellite propagator before trying to update the eci position')
    ...
    def from_state_vector(self, mjd: datetime, orbitnumber: int, r: np.ndarray, v: np.ndarray):
