...
class OrbitalPlaneGrid(object):
    ...
    def __init__(self, opt_geo: List[OpticalGeometry], grid_altitudes, numinbetween=1.0, cutoff=0, extend=0, placementtype='scan',
                 geoid_mode='wgs84'):
        ...
        self._averagescantp, self._averagescanlook, self._averagescanmjd = self._tangent_points(opt_geo)
    ...
    def _make_retrieval_profile_locations(self):
        ...
        """
        rottp = self._correct_for_earths_rotation(self._averagescantp, self._averagescanmjd, self._averagescanmjd[0])
        ...
        for idx in range(1, np.size(localangles)):
            localangles[idx] = np.arctan2(yaxis.dot(rottp[idx, :]), xaxis.dot(rottp[idx, :])) + twopiaddition
        ...
        averageanglechange = totalanglechange / np.size(self._averagescanmjd) * self._numinbetween
        possibleretangles = np.linspace(0, totalanglechange, np.int(np.floor(totalanglechange / averageanglechange)))
        ...
        if self._placementtype == 'scan':
            ...
            possibleretangles = possibleretangles[
                (possibleretangles > self.cutoff) & (possibleretangles < (totalanglechange - self.cutoff))]
            ...
            if self._extend > 0:
                ...
                lower_add = np.arange(num_extend, 0.5, -1) * delta_angle * -1 + possibleretangles[0]
                upper_add = np.arange(1, num_extend + 0.5, 1) * delta_angle + possibleretangles[-1]

                possibleretangles = np.concatenate((lower_add, possibleretangles, upper_add))
        else:
            raise ValueError("Placement type is not one of ['scan', 'uniform']")
        ...
        for idx in range(self._numretprof):
            self._retprofileloc[idx, :] = rotation_matrix(normal, -1 * possibleretangles[idx]).dot(rottp[0, :])

        self._retprofilemjd = np.interp(possibleretangles, localangles, self._averagescanmjd)
        f = interpolate.interp1d(localangles, self._averagescanlook, axis=0, fill_value='extrapolate')  # Extrapolate in case we are extending the profiles
        self._retprofileavglook = f(possibleretangles)

        self._retprofileloc = self._correct_for_earths_rotation(self._retprofileloc, -1 * self._retprofilemjd,
                                                                -1 * self._averagescanmjd[0])
    ...
    def get_local_plane(self, loc: np.ndarray) -> (np.ndarray, np.ndarray, np.ndarray):
        ...
        for idx in range(self._numretprof):
            anglesbetween[idx] = np.arccos(
                loc.dot(self._retprofileloc[idx, :]) / np.linalg.norm(loc) / np.linalg.norm(self._retprofileloc[idx, :]))
        ...
        avglook = (np.gradient(self._retprofileloc, axis=0) / np.linalg.norm(np.gradient(self._retprofileloc, axis=0), axis=1)[:, np.newaxis])[minidx]
        ...
        twopiaddition = 0
        for idx in range(self._numretprof):
            allangles[idx] = np.arctan2(yaxis.dot(rottp[idx, :]), reference.dot(rottp[idx, :]))
            if idx > 0:
                if allangles[idx] < (allangles[idx - 1] - np.pi):
                    twopiaddition += 2 * np.pi
        ...
        if np.size(lowangleidx) == 0:
            lowangleidx = 0
        else:
            lowangleidx = lowangleidx[0]
    ...
    def interpolate_onto_grid(self, prof_lon: np.ndarray, prof_lat: np.ndarray, prof_height: np.ndarray,
                              prof: np.ndarray, do_height_interp=True) -> np.ndarray:
        """
        ...
        if len(s) > 1:
            ...
            for idx in range(s[1]):
                temp_prof[:, idx] = self.distance_interpolate(prof_lat, prof_lon, retlat,
                                                              retlon, prof[:, idx])
            ...
            if do_height_interp:
                ...
                for idx in range(len(retlat)):
                    ret_prof[idx, :] = np.interp(retheight, prof_height, temp_prof[idx, :])
            ...
            temp_prof = self.distance_interpolate(prof_lat, prof_lon, retlat, retlon, prof)
            return temp_prof

    def interpolate_from_grid(self, prof_lon: np.ndarray, prof_lat: np.ndarray, prof_height: np.ndarray,
                              ret_prof: np.ndarray, do_height_interp=True) -> np.ndarray:
        """
        ...
        if len(s) > 1:
            ...
            # profile is (location,height), so loop over heights
            for idx in range(s[1]):
                temp_prof[:, idx] = self.distance_interpolate(retlat, retlon, prof_lat,
                                                              prof_lon, ret_prof[:, idx])
            if do_height_interp:
                # Now construct the retrieval profile by interpolating over height
                meas_prof = np.zeros((len(prof_lat), len(prof_height)))
                for idx in range(len(prof_lat)):
                    meas_prof[idx, :] = np.interp(prof_height, retheight, temp_prof[idx, :])
            ...
            temp_prof = self.distance_interpolate(retlat, retlon, prof_lat, prof_lon, ret_prof)
            return temp_prof

    @staticmethod
    def distance_interpolate(from_lat: np.ndarray, from_lon: np.ndarray, to_lat: np.ndarray, to_lon: np.ndarray,
        ...
        end_lon = from_lon[-1]
        end_lat = from_lat[-1]
        dist_grid_to_end = np.cos(np.deg2rad(end_lat)) * np.cos(np.deg2rad(from_lat)) * np.cos(
            np.deg2rad(from_lon - end_lon)) + np.sin(np.deg2rad(from_lat)) * np.sin(np.deg2rad(end_lat))
        ...
        for idx, (lat, lon) in enumerate(zip(to_lat, to_lon)):
            ...
            distance = np.arccos(np.cos(np.deg2rad(lat)) * np.cos(np.deg2rad(from_lat)) * np.cos(
                np.deg2rad(from_lon - lon)) + np.sin(np.deg2rad(from_lat)) * np.sin(np.deg2rad(lat)))

            # Check if we are outside the grid
            if np.all(np.diff(distance) > 0):
                new_profile[idx] = profile[0]
                continue
            elif np.all(np.diff(distance) < 0):
            ...
            if not (np.all(np.diff(distance[useful_idx]) > 0) or np.all(np.diff(distance[useful_idx]) < 0)):
                logging.warning("Distance between points is not monotonic, using the nearest point instead")
                new_profile[idx] = sp.interpolate.interp1d(distance, profile, kind='nearest')(0.0)

            if distance[useful_idx][1] < distance[useful_idx][0]:
                new_profile[idx] = np.interp([0.0], distance[useful_idx][::-1], profile[useful_idx][::-1])
            else:
                new_profile[idx] = np.interp([0.0], distance[useful_idx], profile[useful_idx])
    ...
    def _correct_for_earths_rotation(tp, mjd, refmjd=None):
        ...
        for idx in range(np.size(mjddiff)):
            ...
            rot_mat = np.array([[np.cos(theta), -1 * np.sin(theta), 0], [np.sin(theta), np.cos(theta), 0], [0, 0, 1]])
